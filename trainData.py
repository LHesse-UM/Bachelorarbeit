...
if not os.path.exists(train_test_val_path):
    ...
    # Einladen der Daten
    csv = pd.read_csv('./trainingsdaten/fahrräder.csv')
    csv2 = pd.read_csv('./trainingsdaten/autos.csv')
    data = pd.concat([csv, csv2])
    
    data['Timestamp'] = pd.to_datetime(data['Timestamp'], format='%H:%M:%S.%f')
    data['Timestamp'] = data['Timestamp'].dt.time

    # Zeitstempel fürs Labeln
    time_pairs = [
    ...
    data_reverse = pd.read_csv('./trainingsdaten/reverseData.csv')
    data_reverse['Timestamp'] = pd.to_datetime(data['Timestamp'], format='%H:%M:%S.%f')
    data_reverse['Timestamp'] = data['Timestamp'].dt.time
    time_pairs_reverse = [
    ...
    # Umkehren der Daten
    X_reverse = data_reverse.iloc[:, :-2].values
    for index, row in data_reverse.iterrows():
        for start, end in time_pairs_reverse:
            ...
            if start_time <= row['Timestamp'] <= end_time:
                data_reverse.at[index, 'Label'] = 1
        
            for j in range(0, len(X_reverse[index]), 8): 
                X_reverse[index, j:j+8] = X_reverse[index, j:j+8][::-1]
            data_reverse.iloc[:, :-2] = X_reverse
    '''
    ...
    if data_reserve:
        data_balanced = pd.concat([data, data_reserve])
    else: 
        data_balanced = data

    '''
    # Optional: Unter-Sampling der überrepräsentierten Klasse
    class_0 = data_balanced[data_balanced['Label'] == 0]
    class_1 = data_balanced[data_balanced['Label'] == 1]
